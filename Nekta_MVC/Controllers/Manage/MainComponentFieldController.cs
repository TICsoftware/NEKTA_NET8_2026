...
namespace Nekta_MVC.Controllers.Manage
{
    [Authorize]
[SessionAuthorize]
    public class MainComponentFieldController : Controller
    {
        private readonly MainComponentField_BAL bal;
        private readonly maincomponentmasterFields_BAL _bal;
        public MainComponentFieldController(IConfiguration config)
        {
            bal = new MainComponentField_BAL(config);
        }
        ...
        [HttpPost]
        public IActionResult Edit(ComponentMasterField model)
        {
            if (ModelState.IsValid)
            {
