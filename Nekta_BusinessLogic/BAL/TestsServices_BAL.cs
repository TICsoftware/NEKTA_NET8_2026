...
{
    public class TestsServices_BAL : Page_Manage_DAL
    {

        private readonly IConfiguration _configuration;
        public TestsServices_BAL(IConfiguration configuration) : base(configuration)
        {
        ...
        // private readonly IConfiguration _configuration;
        // public TestsServices_BAL(IConfiguration configuration)
        // {
        //     //_dal = new Page_Manage_DAL(configuration);
        //     _configuration = configuration;
        // }

        public TestsServicesModel GetTestServices_BAL(string pagename, int languageId, int geographyId)
        {
            var model = new TestsServicesModel();
            ...
            if (ds.Tables.Count > 1 && ds.Tables[1].Rows.Count > 0)
            {
                ...
                model.FindTest_List = MapComponentCommon(groupedData, 1);
                model.Services_List = MapComponentCommon(groupedData, 2);
                model.TestDirectory_List = MapComponentCommon(groupedData, 3);
                model.ReferenceResources_List = MapComponentCommon(groupedData, 4);
                model.DiagnosticRequest_List = MapComponentCommon(groupedData, 5);
        ...
        public TestsServicesModel GetForPAtients_BAL(string pagename, int languageId, int geographyId)
        {
            var model = new TestsServicesModel();
            var ds = GetContentComponentData_DAL(pagename, languageId, geographyId);
            ...
            if (ds.Tables.Count > 1 && ds.Tables[1].Rows.Count > 0)
            {
                ...
                model.BookATest_List = MapComponentCommon(groupedData, 1);
                model.WHyTrust_List = MapComponentCommon(groupedData, 2);
                model.StepToSchedule_List = MapComponentCommon(groupedData, 3);
                model.FAQ_List = MapComponentCommon(groupedData, 4);
                model.Visitlab_List = MapComponentCommon(groupedData, 5);
                model.needhelp_List = MapComponentCommon(groupedData, 6);
            }
