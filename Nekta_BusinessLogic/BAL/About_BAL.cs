...
{
    public class About_BAL : Page_Manage_DAL
    {
        ...
        public AboutModel GetAboutCop_BAL(string pagename, int languageId, int geographyId)
        {
            ...
            if (ds.Tables.Count > 1 && ds.Tables[1].Rows.Count > 0)
            {
                ...
                model.BringingDiagnostic_List = MapComponentCommon(groupedData, 1);
                model.COPWorks_List = MapComponentCommon(groupedData, 2);
                model.TheVision_List = MapComponentCommon(groupedData, 3);
                model.PartnersCollaborators_List = MapComponentCommon(groupedData, 4);
                model.RisingCancer_List = MapComponentCommon(groupedData, 5);
                model.HaveQuery_List = MapComponentCommon(groupedData, 6);
            }
        ...
        public AboutModel GetCareers_BAL(string pagename, int languageId, int geographyId)
        {
            var model = new AboutModel();
            var ds = GetContentComponentData_DAL(pagename, languageId, geographyId);
            ...
            if (ds.Tables.Count > 1 && ds.Tables[1].Rows.Count > 0)
            {
                ...
                model.OpenRoles_List = MapComponentCommon(groupedData, 1);
                model.WorkingWithUs_List = MapComponentCommon(groupedData, 2);
                model.CurrentOpportunities_List = MapComponentCommon(groupedData, 3);
                model.ShareYourProfile_List = MapComponentCommon(groupedData, 4);
            }
        ...
        public AboutModel GetPartnersCollaborators_BAL(string pagename, int languageId, int geographyId)
        {
            var model = new AboutModel();
            ...
            if (ds.Tables.Count > 1 && ds.Tables[1].Rows.Count > 0)
            {
                ...
                model.PartnersCollaborators_List = MapComponentCommon(groupedData, 1);
                ...
                {
                    var groupedData2 = GetGroupedComponents(ds.Tables[3]);
                    model.Components2 = groupedData2;
                    model.HaveQuery_List = MapComponentCommon(groupedData2, 2);
        ...
        public AboutModel GetPartnerscollaborators_BAL(int ContentID, int PageNumber, int PageSize)
        {
